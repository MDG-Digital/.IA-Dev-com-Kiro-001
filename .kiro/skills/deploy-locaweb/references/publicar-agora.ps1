# Prospector de Sites — publicação na Locaweb
...
if (-not (Test-Path "fila-publicacao.txt")) {
	if (-not $Auto) {
		Log "Nada na fila - monte a fila-publicacao.txt primeiro." "Yellow"
	}
	Fim 0
}

try {
	$cfg = Get-Content "prospector-config.json" -Raw -Encoding UTF8 | ConvertFrom-Json
}
catch {
	Log "ERRO: prospector-config.json nao encontrado/invalido." "Red"
	Fim 1
}

$u = $cfg.locaweb.usuario
$p = $cfg.locaweb.senha
$srv = $cfg.locaweb.servidor

if (-not $u -or -not $p -or -not $srv) {
	Log "ERRO: preencha a conexao Locaweb (dashboard > Configuracoes) incluindo a senha." "Red"
	Fim 1
}
...
foreach ($linha in $fila) {
	...
	if (-not (Test-Path $local)) {
		Log ("PULOU (nao existe): " + $local) "Yellow"
		$falha++
		continue
	}
	...
	if ($LASTEXITCODE -eq 0) {
		...
		Log ("  FALHOU (codigo " + $LASTEXITCODE + ")") "Red"
		$falha++
	}
}
...
if ($falha -eq 0 -and $ok -gt 0) {
	...
	Log "Fila concluida. Avise o Kiro ('publiquei') para verificar as URLs." "Cyan"
}
