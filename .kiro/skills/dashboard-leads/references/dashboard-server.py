...
class App(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.split('?')[0] == '/api/config':
            cfg = ler_config()
            # chave nova: 'locaweb' (fallback 'hostgator' para configs antigos)
            hg = dict(cfg.get('locaweb', cfg.get('hostgator', {})))
            hg['senhaDefinida'] = bool(hg.get('senha'))
            hg.pop('senha', None)  # a senha NUNCA sai do arquivo
            # expõe nas duas chaves para compatibilidade do template
            return self._json(200, {'contratante': cfg.get('contratante', {}), 'locaweb': hg, 'hostgator': hg})
    ...
    def do_PUT(self):
        if self.path.split('?')[0] == '/api/config':
            ...
            if 'contratante' in corpo or 'locaweb' in corpo or 'hostgator' in corpo:
                if 'contratante' in corpo:
                    ct = cfg.get('contratante', {})
                    ct.update({k: v for k, v in corpo['contratante'].items() if isinstance(v, str)})
                    cfg['contratante'] = ct
                # aceita 'locaweb' (novo) ou 'hostgator' (legado); grava sempre em 'locaweb'
                conexao_nova = corpo.get('locaweb', corpo.get('hostgator'))
                if conexao_nova is not None:
                    lw = cfg.get('locaweb', cfg.get('hostgator', {}))
                    for k, v in conexao_nova.items():
                        if not isinstance(v, str): continue
                        if k == 'senha' and v == '': continue  # em branco = mantém a atual
                        lw[k] = v
                    cfg['locaweb'] = lw
                    cfg.pop('hostgator', None)  # migra: remove a chave antiga
